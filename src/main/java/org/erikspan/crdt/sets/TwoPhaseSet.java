...
public class TwoPhaseSet<E> implements
	...
	@Override
	public Set<E> value() {
		Set<E> values = new HashSet<E>(this.adds);
      values.removeAll(this.removals);
      return values;
	}
	...
	public byte[] payload() {
		try {
			Map<String, JsonNode> retval = new HashMap<String, JsonNode>();
	...
	public boolean addAll(final Collection<? extends E> col) {
		if (!removals.retainAll(col)) {
			throw new IllegalArgumentException(String.format(
                 "%d Elements have already been removed.", col.size()));
		}
	...
	public boolean containsAll(final Collection<?> col) {
		removals.retainAll(col);
		return removals.isEmpty() && adds.containsAll(col);
	}
	...
	public boolean removeAll(final Collection<?> col) {
      Set<E> intersection = new HashSet<E>(this.adds);
	...
	public boolean retainAll(final Collection<?> col) {
		Set<E> diff = new HashSet<E>(this.value());
      diff.removeAll(diff);
